...
import 'kleur/colors';
import { $ as $$Banner, a as $$Layout } from './Layout_CQhcHKs_.mjs';
...
const $$CaseStudyLayout = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": frontmatter.title }, { "default": ($$result2) => renderTemplate` ${renderComponent($$result2, "Banner", $$Banner, { "showHorse": true, "title": frontmatter.title })} ${maybeRenderHead()}<h1>${frontmatter.title}</h1> <p>${frontmatter.pubDate.toString().slice(0, 10)}</p> <p><em>${frontmatter.description}</em></p> <p>Written by: ${frontmatter.author}</p> <img${addAttribute(frontmatter.image.url, "src")} width="300"${addAttribute(frontmatter.image.alt, "alt")}> ${renderSlot($$result2, $$slots["default"])} ` })}`;
}, "/usr/local/var/www/magenta-magnitude/src/layouts/CaseStudyLayout.astro", void 0);
